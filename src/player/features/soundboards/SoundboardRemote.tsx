...
export function SoundboardRemote({ onPlay, onStop }: SoundboardRemoteProps) {
	...
	useEffect(() => {
		...
		window.player.on("PLAY_AUDIO", (sound) => {
			onPlay(sound);
		});
	...
	useEffect(() => {
		...
		return () => {
			window.player.removeAllListeners("PLAYER_REMOTE_SOUNDBOARD_PLAYBACK_REQUEST");
		};
	}, [playback]);
	...
	useEffect(() => {
		window.player.on("PLAYER_REMOTE_SOUNDBOARD_GET_ALL_REQUEST", () => {
			window.player.soundboardGetAllReply({
				soundboards: soundboards.soundboards.allIds.map((id) => soundboards.soundboards.byId[id]),
